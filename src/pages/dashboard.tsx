...
import { getHrData } from 'apiSdk/hr-data';
...
import { HrDataListPage as ShowCaseList } from 'pages/hr-data';

function DashboardPage() {
  const { session } = useSession();

  const hrDataStat = useSWR<PaginatedInterface<HrDataInterface>>(
    () => `/hr-data`,
    () => getHrData({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Hr Data',
      result: hrDataStat?.data?.totalCount,
      isLoading: hrDataStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`hr-data`} passHref legacyBehavior>
                  <Button
